...
def read_all_stock_files(individual_stocks_5yr):
    allFiles = []
    for (_, _, files) in os.walk('C:\\Users\Dalip Kumar\\Documents\\8th Semester\\Major Project\\Stock market\\send stoke\\individual_stocks_5yr\\'):
        allFiles.extend(files)
        break

    dataframe_dict = {}
    for stock_file in allFiles:
        df = pd.read_csv('C:\\Users\Dalip Kumar\\Documents\\8th Semester\\Major Project\\Stock market\\send stoke\\individual_stocks_5yr\\' + "/" +stock_file)
...
def LSTM_model(dates, prices, test_date, df):
    ...
    test_score = r2_score(y_test[0], mainTestPredict)
